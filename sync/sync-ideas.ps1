...
#   powershell -File sync-ideas.ps1 -BlazorRoot 'D:/path/MindAttic.Ideas.Web'
# ====================================================================
[CmdletBinding()]
param(
    [string]$ContentRoot,
...
if (-not $sub.subscriptions -or @($sub.subscriptions).Count -eq 0) {
    Write-Output "  Ideas: no subscriptions yet -- nothing to sync."
    return
}
...
if (-not (Test-Path $appCss)) { throw "Path not found: $appCss (bootstrap the marker blocks first)" }
...
function Splice-CssMarkerBlock {
    ...
    $block = $Begin + "`r`n" +
             "/* Generated by MindAttic.UiUx/sync/sync-ideas.ps1. Do not edit by hand." + "`r`n" +
...
foreach ($subscription in $sub.subscriptions) {
    ...
    switch ($comp.name) {
        ...
        default {
            throw "Ideas has no dispatch for component '$($comp.name)'. Add a case in sync-ideas.ps1."
        }
    }
}
